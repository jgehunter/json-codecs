...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] =
    Decoder.fromPartialFunction{ case Json.Num(int) if int.isValidInt => int.intValue }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Null => None
      case xs: Json.Arr => if ((xs.items map decoder.decode).exists(_.isEmpty)) None else Option((xs.items map decoder.decode).map(_.get))
    }
...
trait ContactsCodecs {
  ...
  implicit val contactDecoder: Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts](list => Contacts(list))
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val testList = implicitly[Decoder[List[Boolean]]]
